package com.linkapital.core.services.bank_nomenclature.contract.to.get;

import com.linkapital.core.services.bank_nomenclature.contract.enums.CompanyBankDocumentState;
import com.linkapital.core.services.directory.contract.to.DirectoryTO;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Getter;
import lombok.Setter;

import java.util.ArrayList;
import java.util.List;

@ApiModel(description = "The bank nomenclature data.")
@Getter
@Setter
public class CompanyBankDocumentTO {

    @ApiModelProperty(value = "The id.")
    private long id;

    @ApiModelProperty(value = "The description state.")
    private String descriptionState;

    @ApiModelProperty(value = "The state.")
    private CompanyBankDocumentState state;

    @ApiModelProperty(value = "The bank nomenclature.")
    private BankNomenclatureTO bankNomenclature;

    @ApiModelProperty(value = "The directories related to the banking nomenclature.")
    private List<DirectoryTO> directories;

    public CompanyBankDocumentTO() {
        this.directories = new ArrayList<>();
    }

}
